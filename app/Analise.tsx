...
export default function AnaliseScreen() {
  ...
  const { t } = useLanguage(); // CORREÇÃO: Agora dentro do componente
  const [viewMode, setViewMode] = useState<'weekly' | 'monthly'>('weekly');

  // Interface para o banco de dados de simulação (Interna para usar o 't')
  const database = {
    weekly: {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.header}>
        <Text style={styles.title}>{t('analysis_title')}</Text>
...
function MetricItem({ label, value, color, status }: any) {
  return (
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 24, fontWeight: '800', color: '#1B4332', fontFamily: GlobalStyles.fontFamily },
  closeBtn: { backgroundColor: '#FFF', padding: 10, borderRadius: 25, elevation: 3 },
  ...
  tabText: { fontSize: 16, color: '#888', fontWeight: '600', fontFamily: GlobalStyles.fontFamily },
  tabTextActive: { color: '#1B4332', fontFamily: GlobalStyles.fontFamily },
  chartCard: { backgroundColor: '#FFF', borderRadius: 24, padding: 15, elevation: 4, marginBottom: 25 },
  chart: { borderRadius: 16, marginVertical: 8 },
  statsGrid: { gap: 15 },
  metricCard: { backgroundColor: '#FFF', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20, borderRadius: 20, borderWidth: 1, borderColor: '#F0F0F0' },
  metricLabel: { fontSize: 11, color: '#888', fontWeight: 'bold', fontFamily: GlobalStyles.fontFamily },
  metricValue: { fontSize: 20, fontWeight: '700', color: '#333', fontFamily: GlobalStyles.fontFamily },
  badge: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 10 },
  badgeText: { color: '#FFF', fontSize: 10, fontWeight: 'bold', fontFamily: GlobalStyles.fontFamily }
});